...
(async () => {
  ...
  function notifyAll(payload){ const msg = `data: ${JSON.stringify(payload)}\\n\\n`; for(const res of clients){ try{ res.write(msg); }catch{} } }
  ...
  function auth(req, res, next){
    ...
    try{ req.user = jwt.verify(token, JWT_SECRET); next(); }catch(e){ res.status(401).json({ error: 'Token inválido' }); }
  }
  ...
  app.post('/api/auth/register',
    ...
    async (req, res) => {
      ...
      try{
        const result = await db.run(`INSERT INTO companies (name, email, password_hash) VALUES (?,?,?)`, [name, email, hash]);
  ...
  app.post('/api/auth/login',
    ...
    async (req, res) => {
      ...
      const token = jwt.sign({ id: company.id, email: company.email, name: company.name }, JWT_SECRET, { expiresIn: '7d' });
      res.json({ token });
    }
  );
  ...
  app.get('/api/jobs', async (req, res) => {
    ...
    if (q) { clauses.push(`(title LIKE ? OR company LIKE ? OR location LIKE ? OR description LIKE ?)`); params.push(`%${q}%`,`%${q}%`,`%${q}%`,`%${q}%`); }
    ...
    const rows = await db.all(`SELECT * FROM jobs ${where} ORDER BY datetime(created_at) DESC LIMIT ? OFFSET ?`, [...params, limit, offset]);
  ...
  app.post('/api/jobs', auth,
    body('title').isLength({ min: 3 }), body('company').isLength({ min: 2 }),
    body('location').isLength({ min: 2 }), body('description').isLength({ min: 10 }),
    async (req, res) => {
      const errors = validationResult(req);
      if(!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
      const { title, company, location, type='', salary='', description='', tags='',
              urgent=false, no_exp=false, remote=false, apply_url='', apply_email='' } = req.body;
      const result = await db.run(`INSERT INTO jobs
        (title, company, location, type, salary, description, tags, urgent, no_exp, remote, apply_url, apply_email, company_id)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,
        [title, company, location, type, salary, description, tags, urgent?1:0, no_exp?1:0, remote?1:0, apply_url, apply_email, req.user.id]);
  ...
  app.listen(PORT, () => console.log(`ConectaVagas rodando em http://localhost:${PORT}`));
})();
